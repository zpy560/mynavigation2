...
class Map2BaseTf : public rclcpp::Node
{
public:
  Map2BaseTf()
  : Node("map2baseTF")
  {
    ...
    timer_ = create_wall_timer(
      ...
      std::bind(&Map2BaseTf::publishTf, this));

    RCLCPP_INFO(
      get_logger(),
      "Publishing virtual TF %s -> %s at %.3f, %.3f, %.3f yaw %.3f",
      parent_frame_.c_str(), child_frame_.c_str(), x_, y_, z_, yaw_);
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<Map2BaseTf>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
